...
def high_and_low():
    ...
    suite.addTest(RomanNumericalConverterTest("test_parsing_millenia"))
    suite.addTest(RomanNumericalConverterTest("test_parsing_one"))
    return suite

def combos():
    return unittest.TestSuite(map(RomanNumericalConverterTest, ["test_combo1", "test_combo2", "test_combo3"]))

def all():
    return unittest.TestLoader().loadTestsFromTestCase(RomanNumericalConverterTest)
